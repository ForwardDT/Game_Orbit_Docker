...
DOCKING_PROXIMITY_RADIUS = 5      # <-- Maximum distance (in pixels) between ISS and spaceship for docking (modify here)
...
def main():
    ...
    # Initialize Player using the new function
    player_x, player_y, player_vx, player_vy = initialize_player_position()
    ...
    tutorial_tips = [
        ...
        "Press P to toggle orbit prediction, R to restart",
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_r:
                    ...
                    player.fuel = 200
                    player.docked = False
                    player.docking_timer = 0  # Reset docking timer here
                    player.trail = []
                    mission_complete = False
                    tip_index = 0
                elif event.key == K_n:
        ...
        screen.fill(BLACK)
        if show_prediction and not player.docked:
        ...
        altitude = math.sqrt(dx**2 + dy**2) - EARTH_RADIUS
        ...
        controls_text = font.render("W/S: Forward/Back  A/D: Left/Right  P: Prediction  R: Restart", True, WHITE)
